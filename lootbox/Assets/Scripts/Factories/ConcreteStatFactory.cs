...
public class ConcreteStatFactory : StatFactory {
    ...
    public override Stat GetStatObject(Stat.StatType statType, ItemScriptableObject item)
    {
        switch (statType)
        {
            case Stat.StatType.LUCK:
                return new Luck("Luck", Stat.StatType.LUCK, Stat.Rarity.EPIC, Stat.AffectedStat.RARITY, GenerateStatObjectValue(GetItemObjectRarity(item)));
            case Stat.StatType.STRENGTH:
                return new Strength("Strength", Stat.StatType.STRENGTH, Stat.Rarity.COMMON, Stat.AffectedStat.LOOT_AMOUNT, GenerateStatObjectValue(GetItemObjectRarity(item)));
            case Stat.StatType.DEXTERITY:
                return new Dexterity("Dexterity", Stat.StatType.DEXTERITY, Stat.Rarity.RARE, Stat.AffectedStat.ACCUMULATION, GenerateStatObjectValue(GetItemObjectRarity(item)));
            case Stat.StatType.INTELLIGENCE:
                return new Intelligence("Intelligence", Stat.StatType.INTELLIGENCE, Stat.Rarity.COMMON, Stat.AffectedStat.THEME, GenerateStatObjectValue(GetItemObjectRarity(item)));
        }
        throw new System.NotImplementedException();
    }
}
